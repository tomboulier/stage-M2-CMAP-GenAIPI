...
Imaging in Traumatic Brain Injury), mené au CHU de Grenoble. L'objectif à long terme
est de développer des modèles génératifs capables de \textbf{prédire l'évolution des
lésions cérébrales} chez les patients traumatisés crâniens à partir d'images CT
successives. Le traumatisme crânien est la première cause de handicap chez les moins
de 35 ans en Europe~\cite{maas2017tbi}.
...
milliers d'exemples typiquement requis par les modèles génératifs. Cette contrainte
est renforcée par le \textbf{principe de minimisation} du RGPD
(article 5)~\cite{rgpd2016}, qui impose de limiter la collecte de données
personnelles au strict nécessaire.
...
Les modèles de diffusion ont récemment émergé comme une classe particulièrement
performante de modèles génératifs, notamment en imagerie
médicale~\cite{pinaya2022brain,wolleb2022anomaly,khan2025survey}, où ils
permettent de capturer des distributions de données complexes à haute dimension
malgré des volumes d'apprentissage limités. Plusieurs travaux ont montré que ces
modèles pouvaient être adaptés à des régimes de données restreints, en
...
Le modèle étudié sera \textbf{MAISI} (Medical AI for Synthetic
Imaging)~\cite{guo2024maisi}, développé par NVIDIA. MAISI est un modèle de diffusion
latente capable de générer des images CT 3D haute résolution (jusqu'à
$512 \times 512 \times 768$ voxels), pré-entraîné sur environ 39\,000 volumes. Une
version améliorée, MAISI-v2~\cite{zhao2025maisiv2}, propose une accélération d'un
facteur 10 à 33 grâce au \emph{rectified flow}.

L'étude s'appuiera sur le modèle MAISI et le dataset
