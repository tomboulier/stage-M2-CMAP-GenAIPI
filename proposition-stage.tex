...
\hypersetup{
    pdftitle={Apprentissage de modèles génératifs d'images médicales 3D avec peu de données},
    pdfauthor={Josselin Garnier, Thomas Boulier},
    pdfsubject={Proposition de stage de Master 2 -- CMAP, École Polytechnique},
    pdfkeywords={modèles de diffusion, imagerie médicale, few-shot learning, traumatisme crânien}
}

% Bibliographie (natbib doit être chargé AVANT babel-french)
\usepackage[numbers,sort&compress]{natbib}

% Langue : babel-french n'est chargé que s'il est disponible (cf. 'make install'),
% afin que le document compile aussi sur une installation TeX minimale.
\IfFileExists{french.ldf}{%
  \usepackage[french]{babel}%
}{%
  \PackageWarningNoLine{proposition-stage}{%
    babel-french introuvable : la typographie francaise est desactivee.
    Lancez 'make install' pour installer texlive-lang-french}%
}
\usepackage{csquotes}
...
milliers d'exemples typiquement requis par les modèles génératifs. À titre d'exemple,
...
Anesthésie \& Réanimation, 2(6):431--453, 2016.
